...
export const deleteArticle = createAsyncThunk(
  ...
  async (id: string, { getState, rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || "Failed to delete the article");
    }
  }
);
  ...
  extraReducers: (builder) => {
    ...
    //fetch
      ...
      .addCase(deleteArticle.fulfilled, (state, action) => {
        state.articles = state.articles.filter(article => article._id !== action.meta.arg);
      })
